...
class RPCInterface(RyuApp):
    _CONTEXTS = {
        ...
        "switches": topology.switches.Switches,
        "topology_manager": topology.TopologyManager,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rpc_clients = []
        self._topology_manager = kwargs["topology_manager"]
    ...
    def init_client(self, rpc_client):
        fdb = self._router.fdb.to_dict()
        self._rpc_call(rpc_client, "init_fdb", fdb)
        topologydb = self._topology_manager.topologydb.to_dict()
        self._rpc_call(rpc_client, "init_topologydb", topologydb)

    @set_ev_cls(process.EventProcessAdd)
    def _event_process_add_handler(self, ev):
        self._rpc_broadcall("add_process", ev.rank, ev.mac)

    @set_ev_cls(process.EventProcessDelete)
    def _event_process_delete_handler(self, ev):
        self._rpc_broadcall("delete_process", ev.rank)

    @set_ev_cls(router.EventFDBUpdate)
    ...
    @set_ev_cls(ryu.topology.event.EventSwitchLeave)
    def _event_switch_leave_handler(self, ev):
        self._rpc_broadcall("delete_switch", ev.switch.to_dict())

    @set_ev_cls(ryu.topology.event.EventLinkAdd)
    def _event_link_add_handler(self, ev):
        self._rpc_broadcall("add_link", ev.link.to_dict())

    @set_ev_cls(ryu.topology.event.EventLinkDelete)
    def _event_link_delete_handler(self, ev):
        self._rpc_broadcall("delete_link", ev.link.to_dict())

    @set_ev_cls(ryu.topology.event.EventHostAdd)
